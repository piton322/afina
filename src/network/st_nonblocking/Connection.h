...
public:
    Connection(int s, std::shared_ptr<Afina::Storage> ps) 
        : _socket(s), _alive(true), pStorage(ps)
    {
        std::memset(&_event, 0, sizeof(struct epoll_event));
        _event.data.ptr = this;
    }

    inline bool isAlive() const { return _alive; } // периодически вызывается epoll'ом с целью понять, не хотим ли бы убить этот connetion

    void Start(); // вызывается сразу после создания потоком, который делаем epoll, ждем ввода и устанавливаем интерес на чтение

protected:
    // методы вызываются в зависимости от того, что можно сделать с сокетом
    void OnError();
    void OnClose();
    void DoRead();
    void DoWrite();
